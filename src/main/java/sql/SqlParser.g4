...
queryStatement
    : SELECT queryElements  fromClause? orderByClause? limitClause
    ;

queryElements
    :(star='*' | queryElement) (',' queryElement)*
;
...
aggregateExpr
    :  operator=(AVG|MAX|MIN|SUM|AND|OR) '('arguments+')'     # sqlAggregateExpr
    | COUNT '('(starArg='*'|sqlExpr+) ')'                   # sqlCountExpr
    | (
        BIT_AND | BIT_OR | BIT_XOR | STD | STDDEV | STDDEV_POP
        | STDDEV_SAMP | VAR_POP | VAR_SAMP | VARIANCE
      ) '(' aggregator=ALL? functionArg ')'                 # sqlFunctionExpr
    ;
...
sqlExpr
    : left=sqlExpr operator=(PLUS|MINUS|STAR|DIVIDE|AND|OR) right=sqlExpr  #sqlBinaryOpExpr
    | ('(' uid ')'|uid)                                              #sqlIdentifierExpr
    ;
...
expression
    ...
    | NOT? IN '('sqlExpr (','sqlExpr)* ')'                           #sqlInListExpr
